...
begin
  try
    ...
    begin
      ...
      //$XXXXXXXX  method,  line d...d
      //$XXXXXXXX  class__method,  line d...d

      stringValue := Trim(BackTraceStrFunc(addr));
      stringValue := Trim(stringValue.Substring(Pos(' ', StringValue), stringValue.Length));
      stringValue := stringValue.Replace('__', ':', [rfReplaceAll]);
      stringValue := stringValue.Replace(',  line ', ':', [rfReplaceAll]);
      stringValue := stringValue.Replace(' of ', ':', [rfReplaceAll]);
      f.add(stringValue);
      if (fp = nil) then
        break;
      Inc(i);
      if ((i > max_frame_dump)) or (i > 256) then
        break;
    end;
  except
  end;
...
var
  Index: integer;
  sl: TStringList;
  position: TStringArray;
begin
  sl := TStringList.Create;
  fillStack(sl, get_frame, get_pc_addr);
  if sl[6].StartsWith('TLOGLOGGER') then
    position := sl[7].Split(':', 4)
  else
    position := sl[6].Split(':', 4);
  ...
  for Index := 0 to FPatternParts.Count - 1 do
    case TPatternPart(FPatternParts.Objects[Index]) of
      ...
      ppLogger: Result := Result + SysUtils.Format(FPatternParts[Index], [Event.LoggerName]);
      ppClassName: Result := Result + SysUtils.Format(FPatternParts[Index], [position[0]]);
      ppDate: Result := Result + FormatDateTime(DateFormat, Now);
      ppException: Result := Result + SysUtils.Format(FPatternParts[Index], [Event.ErrorMessage]);
      ppFileName: Result := Result + SysUtils.Format(FPatternParts[Index], [position[3]]);
      ppLocation: Result := Result + SysUtils.Format(FPatternParts[Index], [position[2]]);
      ppLine: Result := Result + SysUtils.Format(FPatternParts[Index], [position[2]]);
      ppMessage: Result := Result + SysUtils.Format(FPatternParts[Index], [Event.Message]);
      ppMethod: Result := Result + SysUtils.Format(FPatternParts[Index], [position[1]]);
      ppNewLine: Result := Result + CRLF;
      ppLevel: Result := Result + SysUtils.Format(FPatternParts[Index], [Event.Level.Name]);
      ppRuntime: Result := Result + SysUtils.Format(FPatternParts[Index], [Event.ElapsedTime]);
      ppThread: Result := Result + SysUtils.Format(FPatternParts[Index], [Event.ThreadId]);
      ppNDC: Result := Result + SysUtils.Format(FPatternParts[Index], [Event.NDC]);
...
procedure RegisterClass(ClassType: TClass; InterfaceType: TGUID; const InterfaceName: string; Names: TStringList; Classes: TClassList);
var
  Index: integer;
begin
